<template>
    <div v-for="(index) in pagesCount" class="buttons" :key="index">
        <small-button :title="String(index)" :small="true" :primary="false" @click="changePage(index, $event)" class="pageButton"/>
    </div>
<!--    <small-button :title="'next'"  :primary="false" @click="changePage" class="pageButton"/>-->
...
function  changePage(index, $event){
    ...
    $event.target.style.backgroundColor = 'rgb(115, 196, 255)'
}

// watch(pagesCount, (newValue, oldValue) => {
//     if(newValue > 1) {
//         pagesCount = 1
//     }
// })
